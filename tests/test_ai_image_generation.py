...
class AIImageGenerationTests(unittest.TestCase):
    ...
    def test_generate_ai_images_accepts_provider_neutral_api_key(self):
        ...
        with patch.dict(
            os.environ,
            {"AI_IMAGE_API_KEY": "provider-key", "AI_IMAGE_BASE_URL": "https://images.example/v1"},
            clear=True,
        ):
            with patch("main.generate_ai_image_bytes", return_value=self._png_bytes()) as mocked_generate:
                main.generate_ai_images(config_path_override=rel_config_path)
        ...
        self.assertEqual(mocked_generate.call_args.kwargs["base_url"], "https://images.example/v1")

    def test_generate_ai_images_requires_base_url_with_provider_neutral_key(self):
        config_path = self.tmp_dir / "episode.json"
        self._write_json(
            config_path,
            {
